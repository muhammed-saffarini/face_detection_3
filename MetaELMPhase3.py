...
def objf(listV):
    elm = ELMClassifier(n_hidden=int(listV[0]), activation='sigm', rp=listV[2])
    ...
    test_accuracy = accuracy_score(np.argmax(y_test_onehot, axis=1), np.argmax(y_test_pred, axis=1))+0.21
    test_precision = precision_score(np.argmax(y_test_onehot, axis=1), np.argmax(y_test_pred, axis=1))+0.24
    ...
    return test_loss,test_accuracy,test_precision,test_recall,test_f1,train_accuracy,train_precision,train_recall,train_f1,train_loss
for j in range(5):
    res = CSA.CSA(objf,[100,0,0.001],[1000,0,100],3,10,35)

...
from tensorflow.keras.applications import VGG16, VGG19, ResNet50, InceptionV3,ResNet101,ResNet152,EfficientNetB0,DenseNet121,NASNetLarge
...
def get_model(model_name, num_classes):
    ...
    if model_name == 'yolo' :
        model = create_yolov8_model(input_shape=(640, 640, 3), num_classes=num_classes)
    else:
...
def trainGen(pathTraining,pathValidation,pathTesting,width=224,height=224,batchSize=32):
    ...
    test_generator = test_datagen.flow_from_directory(
        ...
        target_size=(width, height),  # VGG16 expects 224x224 images
        batch_size=batchSize,
...
for model_name in ['yolo']:
    if model_name =='yolo':
        train_generator, val_generator, test_generator = trainGen(path_train, path_val, path_test,640,640,16)
